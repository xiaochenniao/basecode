...
abstract class Smarty_Internal_TemplateCompilerBase {
    ...
    Smarty_Internal_Template $template, $nocache = null, Smarty_Internal_TemplateCompilerBase $parent_compiler = null
    ) {
        // get code frame of compiled template
        $_compiled_code = $template->smarty->ext->_codeFrame->create(
                $template, $this->compileTemplateSource(
                        $template, $nocache, $parent_compiler
        ...
        if (!strpos($variable, '(')) {
            ...
            $this->tag_nocache = $this->tag_nocache |
                    $this->template->ext->getTemplateVars->_getVariable(
                            $this->template, $var, null, true, false
                    )->nocache;
        ...
        if ((string) $text != '') {
            ...
            if ($this->parser->strip) {
                if (strpos($text, '<') !== false) {
                    ...
                    $_offset = 0;
                    if (preg_match_all(
                                    '#(<script[^>]*>.*?</script[^>]*>)|(<textarea[^>]*>.*?</textarea[^>]*>)|(<pre[^>]*>.*?</pre[^>]*>)#is', $text, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER
                            )
                    ) {
                    ...
                    $_offset = 0;
                    if (preg_match_all(
                                    '#@!@SMARTY:([0-9]+):SMARTY@!@#is', $text, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER
                            )
                    ) {
                        foreach ($matches as $match) {
                            $_length = strlen($match[0][0]);
    ...
    public function getPluginFromDefaultHandler($tag, $plugin_type) {
        ...
        $result = call_user_func_array(
                $this->smarty->default_plugin_handler_func, array(
            $tag,
            $plugin_type,
            $this->template,
            &$callback,
    ...
    public function trigger_template_error($args = null, $line = null, $tagline = null) {
        ...
        if (in_array(
                        $this->template->source->type, array(
                    'eval',
                    'string'
                        )
                )
    ...
    public function replaceDelimiter($lexerPreg) {
        return str_replace(
                array('SMARTYldel', 'SMARTYliteral', 'SMARTYrdel', 'SMARTYautoliteral', 'SMARTYal'), array(
    ...
    public function leaveDoubleQuote() {
        if (array_pop($this->_tag_stack_count) !== $this->getTagStackCount()) {
            ...
            $this->trigger_template_error(
                    "unclosed '{{$tag}}' in doubled quoted string", null, true
            );
        }
    }
        ...
        if ($_output !== false) {
            ...
            Smarty::PLUGIN_BLOCK,
                    ...
                    if (isset($this->smarty->registered_plugins[$plugin_type][$tag])) {
                        ...
                        if ($plugin_type === Smarty::PLUGIN_COMPILER) {
                            ...
                            return call_user_func_array(
                                    $this->smarty->registered_plugins[$plugin_type][$tag][0], array(
                                $new_args,
                                $this
                                    )
                            );
                    ...
                    if ($plugin_type === Smarty::PLUGIN_COMPILER && $this->smarty->loadPlugin('smarty_compiler_' . $tag) && (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag($tag, $this))
                        ...
                        if ($function = $this->getPlugin($tag, $plugin_type)) {
                            if (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag($tag, $this)
                                ...
                                return $this->callTagCompiler(
                                                'private_' . $plugin_type . '_plugin', $args, $parameter, $tag, $function
                                );
                            }
                        }
                    }
                ...
                if (is_callable($this->smarty->default_plugin_handler_func)) {
                    ...
                    if ($found) {
                        ...
                        if ($plugin_type === Smarty::PLUGIN_COMPILER) {
                            ...
                            return call_user_func_array(
                                    $this->default_handler_plugins[$plugin_type][$tag][0], array(
                                $new_args,
                                $this
                                    )
                            );
                ...
                if (isset($this->smarty->registered_plugins[Smarty::PLUGIN_COMPILER][$tag])) {
                    ...
                    return call_user_func_array(
                            $this->smarty->registered_plugins[Smarty::PLUGIN_COMPILER][$tag][0], array(
                        $args,
                        $this
                            )
                    );
