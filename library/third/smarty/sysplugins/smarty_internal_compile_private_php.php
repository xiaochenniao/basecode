    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter) {
        ...
        if ($_attr['type'] === 'xml') {
            ...
            $compiler->parser->current_buffer->append_subtree(
                    $compiler->parser, new Smarty_Internal_ParseTree_Tag(
                    $compiler->parser, $compiler->processNocacheCode(
                            "<?php echo '{$output}';?>\n", true
                    )
                    )
            );
        ...
        if ($_attr['type'] !== 'tag') {
            if ($compiler->php_handling === Smarty::PHP_REMOVE) {
                ...
                $output = preg_replace_callback(
                        '#(<\?(?:php|=)?)|(<%)|(<script\s+language\s*=\s*["\']?\s*php\s*["\']?\s*>)|(\?>)|(%>)|(<\/script>)#i', array($this, 'quote'), $_attr['code']
                );
                $compiler->parser->current_buffer->append_subtree(
                        $compiler->parser, new Smarty_Internal_ParseTree_Text($output)
                );
                return '';
                ...
                if (!($compiler->smarty instanceof SmartyBC)) {
                    $compiler->trigger_template_error(
                            '$smarty->php_handling PHP_ALLOW not allowed. Use SmartyBC to enable it', null, true
                    );
                }
            ...
            if (!($compiler->smarty instanceof SmartyBC)) {
                $compiler->trigger_template_error(
                        '{php}{/php} tags not allowed. Use SmartyBC to enable them', null, true
                );
            }
            ...
            return preg_replace(
                    array("#^{$ldel}\\s*php\\s*(.)*?{$rdel}#", "#{$ldel}\\s*/\\s*php\\s*{$rdel}$#"), array('<?php ', '?>'), $_attr['code']
            );
        }
    }
            ...
            if (preg_match(
                ...
                if ($pos > $close) {
                    ...
                    while ($close > $pos && $close < $start) {
                        if (preg_match(
                                        '~' . preg_quote($closeTag, '~') . '~i', $lex->data, $match, PREG_OFFSET_CAPTURE, $from
                                )
                        ) {
                            $close = $match[0][1];
