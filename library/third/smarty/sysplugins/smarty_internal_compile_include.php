...
class Smarty_Internal_Compile_Include extends Smarty_Internal_CompileBase {
    ...
    public function compile($args, Smarty_Internal_SmartyTemplateCompiler $compiler) {
        ...
        if ($merge_compiled_includes) {
            ...
            $tpl = new $compiler->smarty->template_class(
                    trim($fullResourceName, '"\''), $compiler->smarty, $compiler->template, $compiler->template->cache_id, $c_id, $_caching
            );
    ...
    Smarty_Internal_SmartyTemplateCompiler $compiler, Smarty_Internal_Template $tpl, $t_hash
    ) {
        $uid = $tpl->source->type . $tpl->source->uid;
        if (!($tpl->source->handler->uncompiled) && $tpl->source->exists) {
            ...
            if ($tpl->compiled->has_nocache_code) {
                ...
                $compiled_code = str_replace(
                        "{$tpl->compiled->nocache_hash}", $compiler->template->compiled->nocache_hash, $compiled_code
                );
