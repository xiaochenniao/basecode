...
function success(obj) {
    ...
    if (oo("showResult_" + obj.id)) {
        ...
        oo("showResult_" + obj.id).innerHTML = '<img src="/image/yes.jpg">';
    }
}
...
function tx_chk(obj) {
    ...
    if (key.indexOf("/任选@") > -1) {
        ...
        if (names[1].indexOf("/") > -1)
            names[1] = names[1].slice(0, names[1].indexOf("/"))
    ...
    if (key.indexOf("/同意@") > -1) {
        ...
        if (obj.checked == false) {
            ...
            oo("showResult_" + showid).innerHTML = '<img src="/image/yes.jpg">';
    ...
    if (key.indexOf("/手机/") > -1 && !(/^(((13[0-9]{1})|(17[0-9]{1})|(18[0-9]{1})|(15[0-9]{1}))+\d{8})$/.test(val))) {
        oShow.innerHTML = errorMsg("手机格式不正确，正确格式如:13810888888");
        return false;
    }
    ///^[1-9]\d{16}[\d|x|X]$/
    if (key.indexOf("/身份证/") > -1 && !(/^([1-9]\d{13,16}[\d|x|X])$/.test(val))) {
        oShow.innerHTML = errorMsg("身份证格式不正确，请填写15/18位身份证号码");
        return false;
    }
...
function errorMsg(msg) {
    return '<img src="/image/no.jpg"> ' + msg;
}
