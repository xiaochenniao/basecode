        ...
        <div class="admin_table mb10">
            <table width="100%" cellspacing="0" cellpadding="0">
                ...
                <tr class="tr1 vt">
                    ...
                    <td class="td2"><select name="info[<!--{$item.id}-->][usergroup]"<!--{if $item.id==1000}--> disabled="disabled"<!--{/if}-->><!--{html_options options=$groups selected=$item.usergroup}--></select></td>
                    <td class="td2"><!--{$item.logintimes}--></td>
                    <td class="td2"><!--{$item.lastlogintime}--></td>
                    <td class="td2"><!--{$item.lastloginip}--></td>
                    <td class="td2"><!--{if $item.id>1000 || $item.id==$logininfo.id}-->
                        <a href="<!--{$pageurl}-->/edit.do?id=<!--{$item.id}-->" class="fa fa-pencil" title="编辑"></a>
                        <!--{/if}--> <!--{if $item.id>1000}--> <a href="javascript:;" id="manager_<!--{$item.id}-->" onclick="showBox(this.id,{title:'设置“<!--{$item.fullname}-->”的权限',requestType: 'iframe',iframeWH:{width:780,height:420},target:'/sysgroup/purview.do?uid=<!--{$item.id}-->'})" class="fa fa-shield" title="设置权限"></a><!--{/if}-->
                        <!--{if $item.islocked==1}--> <a href="<!--{$pageurl}-->/unlock.do?id=<!--{$item.id}-->")" class="fa fa-unlock-alt" title="解锁"></a><!--{/if}--></td>
                </tr>
                <!--{/foreach}-->
            </table>
        </div>
