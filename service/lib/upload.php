...
class upload {
    ...
    function run($field = 'file', $smallimg = 'true') {
        ...
        if ($is_uploaded_file && !$this->is_allowed_file_type()) {
    ...
    function data() {
        return array(
            ...
            'file_url' => str_replace(ATTACHMENT . "/", "", substr($this->upload_path, strpos($this->upload_path, ATTACHMENT))) . $this->file_name,
    ...
    function set_upload_path($path) {
        $path = trim($path, '/');
        $this->upload_path = UPLOAD_DIR . DIRECTORY_SEPARATOR . $path . '/';
        $tmp = explode('/', $path);
        $checkpath = UPLOAD_DIR . DIRECTORY_SEPARATOR;
        foreach ($tmp as $p) {
            $checkpath .= $p . '/';
    ...
    function set_image_properties() {
        if (strtolower($this->file_ext) == 'bmp') {
            foreach ($this->small_imags as $key => $size) {
                $this->small_imags[$key] = str_replace(ATTACHMENT . "/", "", substr($this->upload_path, strpos($this->upload_path, ATTACHMENT))) . $this->file_name;
            }
            return;
        }
        if (!$this->is_image()) {
        ...
        foreach ($this->small_imags as $key => $size) {
            $maxwidth = $size[0];
            $maxheight = $size[1];
            $ismax = isset($size[2]) && $size[2] == 'max' ? true : false;
            $source_f = str_replace('.' . $this->file_ext, '', $this->file_name) . "_" . $key . "." . $this->file_ext;
            $this->small_imags[$key] = str_replace(ATTACHMENT . "/", "", substr($this->upload_path, strpos($this->upload_path, ATTACHMENT))) . $source_f;
            ...
            if ($this->iswater) {
                ...
                if ($startx > 0 && $starty > 0) {
                    $textcolor1 = imagecolorallocate($im, 255, 255, 255);
                    $textcolor2 = imagecolorallocate($im, 100, 100, 100);
                    $fontfile = APP_DATA_DIR . '/conf/FZS3JW.TTF';
                    imagettftext($newim, 11, 0, $startx + 1, $starty, $textcolor2, $fontfile, "");
                    imagettftext($newim, 11, 0, $startx, $starty, $textcolor1, $fontfile, "");
                }
            }
            switch ($size['mime']) {
                case 'image/jpeg':
            ...
            ImageDestroy($newim);
            if ($ismax) {
                unlink($this->upload_path . $this->file_name); //删除原图
                rename($source, $this->upload_path . $this->file_name); //移动 将max图 更名为原图
                unset($this->small_imags[$key]);
            }
        }
    }
